...
// Deterministic id so every serverless instance produces the same id for the
// same detection — list vs detail requests can land on different instances
// with independent temp state, so timestamp-based ids would diverge.
function deterministicChangeId(key) {
