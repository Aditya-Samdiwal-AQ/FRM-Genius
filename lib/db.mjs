 ...
 *
 * Serverless-friendly: if the bundled `data/` isn't writable (common on
 * read-only serverless filesystems), redirect writes to a per-instance
 * temp dir hydrated once from the bundled `data/` (or `seeds/`).
 */
...
// When the writable data dir is the temp fallback, hydrate it from the
// bundled `data/` (populated at build time by `npm run reset`) so first-
// request users see the full demo state. Falls back to `seeds/`.
